...
function () {
  ...
  function Minutes(minutes, opts) {
    ...
    this.opts.units.m = this.opts.units.m || 'minute'; // Default the time units to display.
    ...
    this.opts.display.m = typeof this.opts.display.m === 'undefined' ? true : this.opts.display.m; // Default pluralize (to true).

    this.opts.pluralize = typeof this.opts.pluralize === 'undefined' ? true : this.opts.pluralize; // Default separation tokens.
  ...
  _createClass(Minutes, [{
    ...
    value: function toString() {
      ...
      var delta = this.minutes,
          parts = []; // Determine the time period.
      ...
      var lastOccurence = new RegExp(Minutes.safeRegExpString(this.opts.tokens.comma) + '(?!.*' + Minutes.safeRegExpString(this.opts.tokens.comma) + ')'); // Join parts with `,`, other than the final one which should be `and`.
    ...
    value: function formatPart(value, unit) {
      var str = value + this.opts.tokens.space + this.opts.units[unit]; // Make the unit representation plural if required.
